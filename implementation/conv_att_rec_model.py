...
import numpy as np
from optimization import nesterov_rmsprop_multiple, dropout_multiple
...
class ConvolutionalAttentionalRecurrentModel(object):
    ...
    def __get_model_likelihood_for_sentence(self, sentence, name_targets, do_dropout=False, dropout_rate=0.5):
        ...
        [h, predictions, token_weights], _ = theano.scan(step, sequences=name_targets, outputs_info=[self.h0, None, None],
                                          ...
                                          strict=True)
        name_log_probs = T.log(T.nnet.softmax(T.dot(predictions, T.transpose(self.all_name_reps[:-1])) + self.name_bias))
        avg_target_name_log_prob = T.mean(name_log_probs[T.arange(name_targets.shape[0] - 1), name_targets[1:]])
    ...
    def __compile_model_functions(self):
            ...
            step_updates, ratios = nesterov_rmsprop_multiple(self.train_parameters, normalized_grads,
                                                    ...
                                                    output_ratios=True)
            step_updates.extend([(v, T.zeros(v.shape,dtype=floatX)) for v in grad_acc[:-1]])
