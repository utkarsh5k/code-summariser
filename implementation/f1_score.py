...
class PointSuggestionEvaluator:
    ...
    def add_result(self, confidence, is_correct, is_unk, precision_recall, unk_word_accuracy):
        #Utility function to encapsulate result
            ...
            for j in xrange(len(rank_to_eval)):
                ...
                if min(n_suggestions, unk_at_rank) > 0:
                    ...
                    if len(unk_accuracies) > 0:
                        ...
                        self.sum_unk_word_locations[j][i] += max(unk_accuracies)

    def get_best_f1(self, suggestions_pr_re_f1):
        """ Pick suggestion with best f1 score """

        max_f1, max_pr, max_re = 0, 0, 0
    ...
    def __str__(self):
        ...
        for i in xrange(len(self.rank_to_eval)):
            ...
            result_string += "Suggestion Frequency " + str(n_made_suggestions[i] / self.num_points) + os.linesep
            result_string += "Suggestion Accuracy " + str(np.divide(n_correct_suggestions[i], n_made_suggestions[i])) + os.linesep
            result_string += "UNK Accuracy " + str(np.divide(self.sum_unk_word_accuracy[i], self.sum_unk_word_locations[i])) + os.linesep
            result_string += "Suggestion Precision " + str(np.divide(self.sum_precisions_suggestions[i], n_made_suggestions[i])) + os.linesep
            result_string += "Suggestion Recall " + str(np.divide(self.sum_recalls_suggestions[i], n_made_suggestions[i])) + os.linesep
            result_string += "Suggestion F1 " + str(np.divide(self.sum_f1_suggestions[i], n_made_suggestions[i])) + os.linesep
    ...
    def get_f1_auc(self, rank_idx = 0):
        ...
        if len(unique_freq) > 0:
            #integral of f1_at_rank wrt unique_freq
    ...
    def get_acc_auc(self, rank_idx=0):
        ...
        if len(unique_freq) > 0:
            #integral of acc_at_rank wrt unique_freq
...
def token_precision_recall(predicted_parts, target_parts):
    ...
    :param target_parts: a list of the target parts
