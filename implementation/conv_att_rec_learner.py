...
from conv_att_rec_model import ConvolutionalAttentionalRecurrentModel
...
class ConvolutionalAttentionalRecurrentLearner:
    ...
    def train(self, input_file, patience=5, max_epochs=1000, minibatch_size=500):
        """ Patience is a cutoff value to check for useless training iterations """
        assert self.parameters is None, "Model is already trained"
        print "Extracting data..."
        # load data
        train_data, validation_data, self.naming_data = FormatTokens.validated_rec_conv_data(input_file, .92, self.padding_size)
...
def run_from_config(params, *args):
    ...
    test_data, original_names = model.naming_data.data_in_rec_conv_format(test_file, model.padding_size)
...
if __name__ == "__main__":
    ...
    with ExperimentLogger("ConvolutionalAttentionalRecurrentLearner", params) as experiment_log:
        ...
        test_data, original_names = model2.naming_data.data_in_rec_conv_format(params["test_file"], model2.padding_size)
        test_name_targets, test_code_sentences = test_data
