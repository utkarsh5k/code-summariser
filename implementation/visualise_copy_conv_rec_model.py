...
if __name__ == "__main__":
    ...
    for i in xrange(len(name_targets)):
        for j in xrange(1, len(name_targets[i])):
            ...
            if j == len(name_targets[i]) -1:
                target_subtoken = learner.naming_data.all_tokens_dictionary.token_from_id(name_targets[i][j])
            else:
            ...
            suggestion_data["is_unk"] = [learner.naming_data.all_tokens_dictionary.is_unknown(token) for token in code[i]]
